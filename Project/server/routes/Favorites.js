...
router.get("/byUserId", validateToken, async (req, res) => {
  const userid = req.user.id;
  const sqlSelect =
    "select f.MenuitemId, m.itemname, m.description, m.price, m.photoURL, m.CategoryID, c.id IS NOT NULL isCart from favorites as f left join menuitems as m on f.MenuitemId = m.id LEFT JOIN cartitems as c on f.MenuitemId = c.MenuitemId where f.UserId= " +
    userid;
